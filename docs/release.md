...
| 任务处理 | 得到唯一双语 SRT |
| 退出 | sidecar 进程随主程序退出 |
| 覆盖升级 | 安装器默认选择“不卸载旧版本”并直接覆盖，保留 `$LOCALAPPDATA\io.github.coconilu.captionnest\models`；这也避免从 `0.2.8` 或更早版本升级时执行旧版的缺陷卸载钩子 |
| 静默/被动/`/UPDATE` | 不显示删除应用数据选择，默认保留模型 |
| 卸载并保留 | 不勾选 Tauri 卸载页的“删除应用数据”，删除程序文件但保留模型 |
| 完整卸载并删除 | 仅在用户明确勾选“删除应用数据”后删除模型及同一应用数据目录；取消卸载不做清理 |
