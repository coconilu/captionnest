...
> **当前构建状态：已发布。** [CaptionNest v0.1.0](https://github.com/coconilu/captionnest/releases/tag/v0.1.0) 已提供 Windows x64 安装包。后续版本可从 `main` 运行 `Windows Release`，由它路由到 `Prepare Release` 创建不可移动的 annotated tag，再由 tag-ref `Windows Release` 构建并生成 GitHub build provenance；也可直接运行 `Prepare Release`。发布后的 Immutable Release 会绑定 tag、commit 和全部 assets。workflow 会从锁定的 PyAV 18.0.0 源码构建自有 wheel，并链接锁定的 LGPL FFmpeg 8.1.2；官方 PyPI wheel 仍会因携带 x264/x265 被门禁拒绝。

可从正式 Release 下载 Windows x64 的 `CaptionNest_*_x64-setup.exe`。维护者在 `main` 手动运行 `Windows Release`（或直接运行 `Prepare Release`）并输入不带 `v` 的版本号；独立的 tag 发布 run 会完成测试、许可证门禁、安装冒烟、build provenance、Draft 资产核对和 Immutable Release。安装器设计为：
