...
| React / Vite 及前端依赖 | 用户界面 | 以 MIT 为主 | 精确版本见 `web/package-lock.json` |
| Tauri 及 Rust crates | Windows 桌面壳 | MIT、Apache-2.0 及其他兼容许可证 | 精确版本见 `src-tauri/Cargo.lock` |
...
依赖版本变化时，以 `uv.lock`、`web/package-lock.json` 与 `src-tauri/Cargo.lock` 为准重新核对许可证。若本文件与第三方组件自带声明冲突，以第三方组件原始许可证和声明为准。
